...
st.set_page_config(page_title="X-Ray AI Dashboard", page_icon="🫁", layout="wide")
...
# Tốt nhất nên dùng st.secrets khi đẩy lên GitHub để bảo mật thông tin
@st.cache_resource
def get_db():
    # Sử dụng st.secrets trên Streamlit Cloud, hoặc gán thẳng URL nếu test local
    try:
...
def load_live_predictions(limit=50):
    docs = list(db["predictions"].find(
        {}, {"_id": 0}
    ).sort("processing_timestamp", -1).limit(limit)) # Sort theo thời gian thực tế
...
st.markdown('<h1 style="text-align:center;color:#1565C0;">🫁 Hệ Thống Cảnh Báo X-Quang Tức Thời</h1>', unsafe_allow_html=True)
    ...
    if df_live.empty:
        ...
        if df_emg.empty:
            ...
            for _, r in df_emg.head(5).iterrows():
                ...
                st.markdown(f'''
                <div class="alert-box">
                  <b>🔴 Bệnh nhân ID: {r.get("patient_id","")} — {diseases_str}</b> (Phát hiện {time_ago} giây trước)<br>
                  <small>Độ nghiêm trọng: <b>{r.get("severity","").upper()}</b> | Tuổi: {r.get("age","")}</small>
                </div>
                ''', unsafe_allow_html=True)
...
with tab_roi:
    ...
    # Tính dựa trên dữ liệu tổng của tất cả các ngày Batch
